...
class MA_UCMEC_dyna_noncoop_hierarchical_peruser(object):
    ...
    def compute_interval_reward(self):
        ...
        if len(self._interval_delays) == 0:
            return 0.0
        D_interval = np.array(self._interval_delays, dtype=np.float32)
        overall_DSR = float(np.mean(D_interval <= self.tau_c))
        mean_norm = float(np.mean(D_interval) / self.tau_c)
        p95 = float(np.percentile(D_interval, 95))
        ...
        '''
        offload_ratio = self._segment_offload_count / max(1, self._segment_delay_count)
        offload_eff = float(np.mean(self._segment_offload_success_ratio * offload_ratio))
        reward = (
            1.0 * overall_DSR
            + 0.5 * offload_eff
            - 0.05 * p95_norm_clip
