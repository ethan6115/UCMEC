...
class MA_UCMEC_dyna_noncoop_hierarchical_peruser(object):
    def __init__(self, render: bool = False, seed=None):
        ...
        # obs = beta(cn) + ap_mask(cn) + delay/uplink/front/satisfy/offload(5)
        #       + pos(2) + speed(1) + front_stats(K*cn) + cpu_pref(K) + local_pref(1)
        self.high_obs_dim = (self.K + 2) * self.candidate_n + (self.K + 9)
        ...
        # r in [0, 10e8]
        self.norm_factor = np.array([819200.0, 1000.0, 3.0, self.P_max, self.max_delay, 10.0])   #對obs做正規化用的，根據論文修改100000改為819200
    ...
    def compute_interval_reward(self):
        ...
        avg_front_off = self._segment_front_sum / offload_den

        lambda_front = 3.0
        reward = -(avg_total + lambda_front * avg_front_off).astype(np.float32)
        return reward
    ...
    def get_global_obs(self):
        ...
        count = max(1, self._segment_delay_count)
        offload_ratio = (self._segment_offload_count / count).reshape(self.M_sim, 1).astype(np.float32)
    ...
    def step_low(self, action):
        ...
        front_delay = np.minimum(front_delay, self.max_delay)  # clip to avoid extreme values
