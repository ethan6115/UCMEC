...
class MA_UCMEC_dyna_noncoop_hierarchical_alluser(object):
    def __init__(self, render: bool = False, seed=None):
        ...
        self.obs_dim = 9  # set the observation dimension of agents
        self.action_dim = 10
        self._render = render
        ...
        self.high_action_space = spaces.Discrete(len(self.cluster_size_candidates))
        ...
        # r in [0, 10e8]
        self.norm_factor = np.array([819200.0*5, 1000.0/5, 1.0, 1.0, 1.0, 1.0, self.P_max, 1.0, 10.0])   #對obs做正規化用的，根據論文修改100000改為819200
    ...
    def set_high_action(self, action_id):
        # Reason: accept raw index or one-hot without runner-side semantics.
        if isinstance(action_id, (list, np.ndarray)):
            action_id = int(np.argmax(action_id))
        else:
            action_id = int(action_id)
        action_id = max(0, min(action_id, len(self.cluster_size_candidates) - 1))
    ...
    def apply_high_action(self, action_id=None):
        ...
        self.current_cluster_size = self.cluster_size_candidates[int(action_id)]
        self.last_cluster_size = self.current_cluster_size
        self._segment_delay_sum = np.zeros((self.M_sim,), dtype=np.float32)
        self._segment_uplink_sum = np.zeros((self.M_sim,), dtype=np.float32)
        self._segment_front_sum = np.zeros((self.M_sim,), dtype=np.float32)
    ...
    def get_global_obs(self):
        ...
        cluster_norm = np.full((self.M_sim, 1), self.last_cluster_size / 10.0, dtype=np.float32)
    ...
    def cluster(self):
        ...
        for i in range(self.M_sim):
            ...
            chosen = sorted_idx[:self.current_cluster_size]
    ...
    def reset(self):
        ...
        sub_agent_obs = []
        # Init low-level obs with local-processing one-hot.
    ...
    def step_low(self, action):
        ...
        # High-level reward source: total delay + deadline exceed penalty.
