...
class MA_UCMEC_dyna_noncoop(object):
    def __init__(self, render: bool = False):
        ...
        self.C_edge = np.random.uniform(10e9, 20e9, [self.K, 1])  # computing resource of edge server in CPU #由10~20增加五倍
        ...
        self.obs_dim = 7  # set the observation dimension of agents
        self.action_dim = 10
        self._render = render
        ...
        # r in [0, 10e8]
        self.norm_factor = np.array([819200.0, 1000.0, 3.0, self.P_max, 2.0, 900, 900])   #對obs做正規化用的，根據論文修改100000改為819200，改為加入位置的版本
    ...
    def reset(self):
        ...
        if self.is_mobile:
            ...
            self.user_dest = np.random.random_sample((self.M, 2)) * 900  

            # 每個 user 的速度（整段 waypoint 期間固定）
            self.user_speed = np.random.uniform(min_speed, max_speed, (self.M, 1))
        else:
            self.user_dest = None
            self.user_speed = None
        
        #每episode固定shadowing
        kappa_1 = np.random.randn(1, self.N)
        kappa_2 = np.random.randn(self.M, 1)
        ...
        for i in range(self.agent_num):
            ...
            0,
            0,
            self.locations_users[i, 0], 
            self.locations_users[i, 1]])    #加入位置資訊
    ...
    def step(self, action):
        ...
        # 根據論文修改速度 5–15 改成 10–20 m/s
            ...
            for i in range(self.M):
                ...
                if dist <= 1e-6:
                    ...
                    self.user_dest[i, :] = np.random.random_sample(2) * 900
                    self.user_speed[i, 0] = np.random.uniform(min_speed, max_speed)
                    continue
        ...
        kappa_1 = np.random.randn(1, self.N)  # 形狀: (1, N)    從rand改成randn，才符合論文的公式
        kappa_2 = np.random.randn(self.M, 1)  # 形狀: (M, 1)，轉置以便廣播  從rand改成randn，才符合論文的公式
        ...
        for i in range(self.K):
            ...
            # --- [關鍵修改 1] 設定縮放因子 (例如 1e9，把 Hz 轉為 GHz) ---
            SCALE_FACTOR = 1e9  
            
            # 定義變數：現在 C_scaled 代表 "多少 GHz" (數值約 1~100)
            C_scaled = cp.Variable(len(serve_user_id))
            
            # --- [關鍵修改 2] 對 Task 也做縮放 ---
            # 原本公式: Delay = Task / C
            # 新公式:   Delay = Task / (C_scaled * SCALE) = (Task / SCALE) / C_scaled
            # 這樣 Delay 的物理數值(秒)不變，但 solver 處理的數字變漂亮了
            task_scaled = np.array(serve_user_task) / SCALE_FACTOR
            _process_delay = cp.multiply(task_scaled, cp.inv_pos(C_scaled))
            
            _local_delay = np.array(_local_delay)
            _uplink_delay = np.array(_uplink_delay)
            
            # 加入懲罰項 (建議 alpha 小一點，例如 1e-3，因為現在數值很敏感)
            alpha = 1e-3 
            
            # 目標函數不變
            func = cp.Minimize(
                cp.sum(cp.maximum(_local_delay, _uplink_delay + _process_delay)) + 
                alpha * cp.sum(_process_delay)
            )
            
            # --- [關鍵修改 3] Constraint 也要除以 SCALE ---
            # 總 CPU (Hz) / SCALE = 總 CPU (GHz)
            max_c_scaled = self.C_edge[i, 0] / SCALE_FACTOR
            cons = [C_scaled >= 0, cp.sum(C_scaled) <= max_c_scaled]
            
            prob = cp.Problem(func, cons)
            
            # 建議加上 solver 選項，並檢查狀態
            try:
                #prob.solve(solver=cp.SCS, verbose=False)
                prob.solve(solver=cp.CLARABEL, tol_gap_abs=1e-4, tol_gap_rel=1e-4, verbose=False)   #改用更快的求解器
            except cp.error.SolverError:
                pass # 讓它進下面的 if check 處理 fallback

            # --- [關鍵修改 4] 還原 C 的數值 (GHz -> Hz) ---
            if prob.status == 'optimal' or prob.status == 'optimal_inaccurate':
                _C = C_scaled.value * SCALE_FACTOR
                # 再次確保沒有負值 (數值誤差修正)
                _C = np.maximum(_C, 1e-6)
            else:
                _C = None
            ...
            for k in range(len(serve_user_id)):
                if _C is not None: 
                    ...
                    actual_C[serve_user_id[k], i] = self.C_edge[i, 0] / len(serve_user_id)
                '''
                #測試cpu分配是否主導 offloading delay
                if _C is not None:
                    u = serve_user_id[k]           # user index
                    Fi = serve_user_task[k]        # 這個 user 的任務量 F_i
                    C_edge_alloc = _C[k]           # 分到的 edge CPU
                    C_user_i = self.C_user[0, u]   # 本地 CPU 能力
                    # 假想「完全在本地跑」的 processing delay（不含 uplink/front）
                    hypo_local_proc = Fi / C_user_i
                    # 實際 edge processing delay（你等一下會用來算 actual_process_delay）
                    edge_proc = Fi / max(C_edge_alloc, 1e-9)
                    # uplink delay（這顆 CPU 迴圈裡的 _uplink_delay[k]）
                    upl = _uplink_delay[k]
                    #這個 user 真正 offload 的「雲端總 delay」：uplink + edge_proc
                    edge_total = upl + edge_proc
                    # 只印出「offload 反而更慢」的 case，避免 log 爆炸
                    if self.step_num % 100 == 0 and edge_total > hypo_local_proc:
                        print(f"[DEBUG step {self.step_num}] user {u} on CPU {i}")
                        print(f"  Task F_i = {Fi:.3e}")
                        print(f"  C_user   = {C_user_i:.3e} Hz")
                        print(f"  C_edge_alloc   = {C_edge_alloc:.3e} Hz")
                        print(f"  C_edge   = {self.C_edge[i, 0]:.3e} Hz")
                        print(f"sum C on CPU {i} = {np.sum(_C):.3e}, total C_edge = {self.C_edge[i,0]:.3e}")
                        print(f"  uplink_delay       = {upl*1e3:.2f} ms")
                        print(f"  hypo_local_proc    = {hypo_local_proc*1e3:.2f} ms")
                        print(f"  edge_proc_delay    = {edge_proc*1e3:.2f} ms")
                        print(f"  edge_total_delay   = {edge_total*1e3:.2f} ms")
                        print("-"*60)
                '''
        ...
        for i in range(self.agent_num):
            ...
            self.p_last[i],
            self.delay_last[i, 0],
            self.locations_users[i, 0], #加入位置資訊
            self.locations_users[i, 1]
            ])
